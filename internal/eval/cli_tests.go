...
func RunLogaggTests(projectDir, fixturesDir string) (*TestResult, error) {
	...
	r.RunTest("parse Apache shows status", fmt.Sprintf("%s parse %s --format apache 2>&1 || %s parse %s 2>&1", r.Binary, apacheLog, r.Binary, apacheLog), "200")
	...
	r.RunTest("stats command runs", fmt.Sprintf("%s stats %s 2>&1", r.Binary, jsonLog), "")

	// Query tests (optional)
	fmt.Println("")
	fmt.Println("🔎 Query Tests")
	if r.CommandExists(fmt.Sprintf("%s query --help 2>&1 | grep -q 'query\\|SELECT'", r.Binary)) {
		r.RunTest("query SELECT works", fmt.Sprintf("%s query %s \"SELECT level FROM logs\" 2>&1", r.Binary, jsonLog), "info")
	} else {
		fmt.Println("  ⏭️  SKIP: query command not implemented")
	}
